...
def generate_voice(text: str, output_path: Path, language: str = "en") -> None:
    """Generate narration with remote Pocket by default, or local Pocket when requested."""
    del language
    output_path.parent.mkdir(parents=True, exist_ok=True)
    provider = os.getenv("TTS_PROVIDER", "pocket_remote").strip().lower()
    if provider in {"pocket_remote", "remote"}:
        _generate_remote_pocket_voice(text, output_path)
    elif provider in {"pocket", "local"}:
        _generate_local_pocket_voice(text, output_path)
    else:
        raise TTSError(f"Unknown TTS_PROVIDER {provider!r}; use pocket_remote or pocket.")
